...
classes = ['Persona enojada', 'Con miedo', 'Neutral', 'sorprendido', 'Triste', 'Sorprendido']

# Función para abrir la aplicación y activar la cámara
def abrir_aplicacion():
    cap = cv2.VideoCapture(0, cv2.CAP_DSHOW)
    return cap

# Función para predecir emociones en los rostros detectados
...
# Abre la ventana de inicio para abrir la aplicación
while True:
    cv2.namedWindow("Inicio", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("Inicio", 400, 200)
    inicio = np.zeros((200, 400, 3), dtype=np.uint8)
    cv2.putText(inicio, "Presiona 'S' para abrir la camara", (20, 80), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
    cv2.imshow("Inicio", inicio)
    key = cv2.waitKey(1) & 0xFF
    if key == ord('s'):
        break

# Abre la cámara
cam = abrir_aplicacion()
...
while True:
    ...
    # Agregamos una imagen de fondo para el texto de FPS
    bg = np.zeros((50, 200, 3), dtype=np.uint8)
    cv2.putText(bg, str(int(fps)) + " FPS", (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 255, 255), 2, cv2.LINE_AA)
    frame[10:60, 10:210] = bg
